---
title: "Week 10: Flexible Regression"
...
- **Finite mixture models**
  - Model observed data as a **mixture** of discrete latent **categories**
  - Inference via **EM**
...
- **Week 11** - Flexible Regression: Kernels and Forests
- **Week 12** - Flexible Regression: Gaussian Processes
- **Week 13** (No Class Wednesday) - Open Topic!
- **Week 14** - Semiparametric Inference
- **Week 15** - Big, Fast Regressions

- **Assignments**
  - Problem Set 4 - Assigned November 11th, Due December 1st
  - Final Exam - Assigned December 8th, Due December 15th
...
- Our "smoothing spline" takes the form of a **penalized** optimization problem. We want to find the function $f()$ that minimizes:

$$\sum_{i=1}^n (Y_i - f(X_i))^2 + \lambda \int \bigg(f^{\prime\prime}(x)\bigg)^2dx$$
...
```{r}
pa_x <- model.matrix(durationDays ~ as.factor(region) + bs(year, df=8) + as.factor(incompatibility) +
...
```{r}
conflict_lasso_cv <- cv.glmnet(x=pa_x, y=pa$durationDays, family="gaussian", alpha=1)
...
```{r}
conflict_lasso_best <- glmnet(x=pa_x, y=pa$durationDays, family="gaussian", alpha=1, lambda=conflict_lasso_cv$lambda.min)
```
