---
title: "Week 11: Flexible Regression - Part 2"
...
$$\hat{f} = \argmin_{f} \ \mathcal{L}\bigg(\{x_i, y_i\}_{i=1}^n\bigg)$$
...
- We can write $K(x, y)$ as an eigen-function expansion:

$$K(x, x^\prime) = \sum_{i=1}^\infty \gamma_i \phi_i(x)\phi_i(x^\prime)$$

- where

$$\phi(x) = \sum_{i=1}^\infty c_i \phi_i(x)$$



# Reproducing Kernel Hilbert Spaces

- With some regularity conditions on $c_i$ and $\gamma_i$, we can "represent" an evaluation of $\phi(x)$ by way of an inner product  

$$\phi(x_i) = \langle \phi, K(\cdot, x_i) \rangle$$
...
- An alternative way of thinking about the task of predicting $Y$ from $X$ is in terms of **decision trees**
  - Given some input $X$ can we come up with a "flow chart" that will generate a good predicction for $Y$ based on a series of yes/no splits?


- Each **tree** is a sequence of **nodes**
  - At each **node**, we split the tree based on the observed value of a particular covariate
  - Continue to follow the path of the tree (splitting on other covariates) until we reach the bottom of the tree
  - Output is either a **classification** (binary/discrete outcomes) or an **expectation** of $Y$ (regression)


- The tree **partitions** the space into $M$ regions $R_1, R_2, R_3, \dotsc, R_M$.
  - Our predicted value of $Y$ given input $X$ is
  
$$f(X) = \sum_{m=1}^M c_m \mathbf{I}(X \in R_m)$$

where $c_m$ is our "best prediction" for Y in a given bin (typically the mean for squared loss)



# Recursive partitioning

- How do we figure out the partitions for a given tree?
  ...
  - (4): Continue until stopping conditions reached
  
- How large should we grow the tree?
  - If we split on every covariate, we're likely to **overfit**!
  - Could tune **tree depth** but inefficient splits could lead to more efficient ones


- **Cost-Complexity** pruning
  - (1): Grow a **large** tree until we hit some minimum node size (e.g. all nodes have fewer than 5 observations)
  - (2): Recombine splits that don't improve fit above some minimum threshold



# Decision trees

- A popular package for implementing tree-based methods is `caret` (Classification and Regression Training)
  - Contains wrapper functions for various methods and makes optimizing over the tuning parameters a lot easier!
...
- We'll start by just showing **one** regression tree applied to our temperature data applying the recursive partitioning algorithm
  - Use the default training parameters

```{r, warning=F}
temp_tree <- rpart(deviation ~ lat + lon,
                  data = weather)
```
...
rattle::fancyRpartPlot(temp_tree, sub="")
```
...
- Predict for each of our grid squares.

```{r}
grid_points_within_us$dtpredict <- predict(temp_tree, newdata= grid_points_within_us)
```
...
ggplot(data = grid_points_within_us) +
  ...
  geom_sf(data = us_shapefile, fill = "white", color = "black") +
  # Plot the heatmap with grid squares
  geom_tile(aes(x = lon, y = lat, fill = dtpredict), width = lon_interval, height = lat_interval) +
  scale_fill_viridis_c() +  # You can change the color scale as needed
  coord_sf(xlim = c(lon_min, lon_max), ylim = c(lat_min, lat_max), expand = FALSE) +
...
- A single tree outputs a sharp classification boundary
  - Potentially noisy predictions!
  - And the relationship between $Y$ and $X$ may be smooth!
  - And we might be concerned that a single tree **overfits** to the data.


- Solution: **Grow a forest of trees** and **average** over the predictions!


- **Random forests** (Breiman, 2001) improve on individual trees using two techniques:
  - **Bagging** (bootstrap aggregating) - Grow separate trees on bootstrap samples of the data
  - **De-correlating** - Use random subsets of *features* in each tree.


- Our predictions are an **average** of predictions across trees
  - Can get arbitrarily smooth prediction surfaces given a larger and larger forest
  - Model evaluation via **out-of-bag** predictions (evaluate on observations that *don't* get sampled in each bootstrap)



# Random forests

- Let's instead grow **many trees** - each tree is grown on a bootstrapped version of the data and generates a different set of splits!

```{r, cache=T}
set.seed(60639)
temp_rf <- train(deviation ~ lat + lon,
                  data = weather, method = "ranger")
```
...
- Predict using the random forests for each of our grid squares.

```{r}
grid_points_within_us$rfpredict <- predict(temp_rf, newdata= model.matrix(~lat + lon, data=grid_points_within_us)[,-1])
```
...
ggplot(data = grid_points_within_us) +
  ...
  geom_sf(data = us_shapefile, fill = "white", color = "black") +
  # Plot the heatmap with grid squares
  geom_tile(aes(x = lon, y = lat, fill = rfpredict), width = lon_interval, height = lat_interval) +
  scale_fill_viridis_c() +  # You can change the color scale as needed
  coord_sf(xlim = c(lon_min, lon_max), ylim = c(lat_min, lat_max), expand = FALSE) +
  labs(title = "Random forest interpolation",
       fill = "Deviation") +
  theme_minimal()
```
...
- Random forests are particularly valuable in settings with a **large number** of covariates where we're concerned about over-fitting.
  - Let's go back to our **UCDP Peace agreements** example
...
- We'll fit a random forest model with all of the 55 predictors

```{r, cache=T}
set.seed(60639)
pa_rf <- train(durationDays ~ as.factor(region) + year + as.factor(incompatibility) +
                       ...
                       as.factor(out_iss) + as.factor(frame), 
              importance = 'impurity',
              data=pa,
              method = "ranger")
```
...
- With many covariates, we may want to know which variables are most **relevant** for making predictions.


- Two approaches
  - **Impurity** - How much do all the splits on a given variable improve prediction quality (e.g. outcome variance)
  - **Permutation** - If we reshuffled the values of a particular variable, how much would it reduce predictive accuracy?


- We can access this with the `varImp` function in `caret` if we fit a model that returns an importance criterion
  
```{r}
pa_varimportance <- varImp(pa_rf)
```
...
```{r, echo=F, warning=F, message=F, fig.align="center", fig.width=9, fig.height=5, dpi=150}
plot(pa_varimportance, xlab="Impurity Importance")
```
...
- One downside with "impurity" is that it will tend to favor covariates with a lot of levels (see the high weight on "year" above)
  - Let's compare to "permutation"

```{r, cache=T}
set.seed(60639)
pa_rf_perm <- train(durationDays ~ as.factor(region) + year + as.factor(incompatibility) +
                       cease + intarmy + ddr + withd + mil_prov + pp + intgov + intciv +
                       elections + interim + natalks + shagov + pol_prov + aut + fed + ind +
                       ref + shaloc + regdev + cul + demarcation + locgov + terr_prov + amn +
                       pris + recon + return + justice_prov + reaffirm + outlin + pko + gender +
...
```{r, echo=F, warning=F, message=F, fig.align="center", fig.width=9, fig.height=5, dpi=150}
plot(varImp(pa_rf_perm), xlab="Perutation Importance")
```
...
- We may want to get a sense of how the predictions change with respect to a given predictor
  - What is the **marginal density** of $Y$ with respect to some predictor $X_i$, averaged over all other predictors $X_{-i}$?


- The `pdp` package allows us to do this for `ranger` objects
...
- `partial` will generate a data frame of predictions for each value of our chosen predictor variable

```{r}
partial_year <- partial(pa_rf, 
pred.var = c("year"), 
...
```{r, echo=F, warning=F, message=F, fig.align="center", fig.width=9, fig.height=5, dpi=150}
partial_year %>% ggplot(aes(x=year, y=yhat)) + geom_line() + theme_minimal() +
  labs(x = "Year",
       y = "Predicted agreement duration",
