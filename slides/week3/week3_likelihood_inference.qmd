...
- Or alternatively
  ...
  $$\ell(\beta, \sigma^2 | \mathbf{y}) =  \frac{1}{n} \sum_{i=1}^n  \bigg[-\frac{1}{2\sigma^2} (y_i - x_i^\prime\beta)^2 - \log(\sigma)\bigg]$$
...
- Let's implement the Normal MLE from scratch
...
election$voteshare_norm <- (election$voteshare - mean(election$voteshare))/sd(election$voteshare)
election$pctWhiteNH_norm <- (election$pctWhiteNH - mean(election$voteshare))/sd(election$voteshare)
```

- Start by writing a function that returns the log-likelihood (evaluated at each observation)

```{r, echo=T}
norm_loglik <- function(pars, Y, X){
  beta <- pars[-length(pars)]
  sigma <- exp(pars[length(pars)]) # Enforce positive value
  return(-(1/(2*(sigma^2)))*(Y - X%*%beta)^2 - log(sigma))
}
```

- We'll use the `maxLik` package which includes some useful bindings to the built-in R optimization routines
...
- The `maxLik` function takes as input a function whose first argument is a vector of the parameters being optimized over.

```{r, echo=T}
estimates <- maxLik(norm_loglik, start = c(rep(0, ncol(X_matrix)), exp(1)),
                   method="NR", # Newton-Raphson optimization
                   Y = election$voteshare,
                   X = X_matrix)
print(estimates)
```
...
- Let's pull the estimates and compare them to OLS!

```{r, echo=T}
beta_MLE <- estimates$estimate[-length(estimates$estimate)]
sigma_MLE <- exp(estimates$estimate[length(estimates$estimate)])
print(beta_MLE)
```
...
```{r, echo=T}
sigma_MLE
sqrt((1/ols_est$df)*sum(residuals(ols_est)^2))
```
...
- Our first step will be to take a first-order **taylor approximation** of the score function around the true MLE $\theta_0$ and write it as a function of our estimator $\hat{\theta}$
  ...
  $$\ell^{\prime}(\hat{\theta}|\mathbf{y}) \approx \ell^{\prime}(\theta_0 | \mathbf{y}) + \ell^{\prime\prime}(\theta_0|\mathbf{y})(\hat{\theta} - \theta_0)$$
...
- Since unbiasedness is true for every value of $\theta$, differentiating with respect to $\theta$ should also give us zero
...
- The **Cauchy-Schwarz Inequality** for expectations states that for two random variables $X$ and $Y$
...
# Illustration: Normal Model

- How do we estimate the sampling variance?
  - Plug in the **sample** inverse negative hessian evaluated at the MLE - again, Law of Large Numbers means its consistent for the population expectation
  - Requires us to take second-order partial derivatives; `maxLik` does it numerically - beware of instability here!

```{r, echo=T}
vcov_normal <- solve(-hessian(estimates))
```

```{r, echo=T}
se_est <- sqrt(diag(vcov_normal))
print(beta_MLE)
print(se_est)
```
...
- Let's return to our result on the asymptotic variance of $\hat{\theta}$. Denote the **score** for each observation at the true parameter value

  $$g_i(y_i | \theta_0) \equiv \frac{\partial}{\partial\theta} \log f(y_i | \theta_0)$$
  
- And the **hessian**
  
  $$h_i(y_i | \theta_0) \equiv \frac{\partial^2}{\partial\theta^2} \log f(y_i | \theta_0)$$
...
- Previously, we used the fact that the variance of the score was equivalent to the information to simplify further.

- But we can instead just estimate each component directly using both the observed hessian **and** the outer product of the observed gradient at each observation
...
- These estimators are **consistent** for the asymptotic variance under mis-specification of the outcome distribution
  - But finite-sample properties can be not great - usually *undercoverage* in small samples.
...
- `estfun` in the `sandwich` library gives us the gradient at each observation of the MLE if we've written the likelihood function to return an $N$-length vector with the likelihood at each observation.

```{r, echo = T}
grad <- sandwich::estfun(estimates)
head(grad)
colMeans(grad)
```
...
```{r, echo=T}
robust_vcov <- solve(-hessian(estimates))%*%(t(grad)%*%(grad))%*%solve(-hessian(estimates))
sqrt(diag(robust_vcov))
sqrt(diag(vcov_normal))
```
