...
- **Multilevel/Hierarchical models**
  - Impose **structure** on parameters by way of prior distributions
  - What structure?
    - Information from **grouping** (partial pooling/'random effects')
    - Information from **time** (random-walk prior)
  - Strength of prior governs the degree of pooling/smoothing
...
- **Challenges with Bayes** - Sampling-based inference can be tricky with GMMs due to the label-switching problem.
  ...
  - Stan doesn't work with discrete variables (can't take the gradient for HMC)
...
- The ordering of words is ignored - the "**bag of words**" assumption
  ...
  - "Modern" extensions would probably use word embeddings
...
```{r, warning=F, message=F, results=F, cache=T, echo=T}
